...
<Sidenote note={`[Plx](https://muffinman.io/react-plx/) uses the
same scroll manager so I extracted it to the standalone package.`}>Scroll manager</Sidenote>
...
### withWindow decorator
  ...
  <Sidenote note={`User needs to pass breakpoints definition.`}>breakpoint</Sidenote>

### How do they work?

Both scroll and resize decorators are listening to custom events,
rather than the native ones. This is because both
<Sidenote note={`Especially scroll event.`}>`scroll` and `resize` events</Sidenote>
