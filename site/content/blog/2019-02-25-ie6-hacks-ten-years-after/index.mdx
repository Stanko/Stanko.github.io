...
> <Sidenote note={`Note that my syntax highlighter didn't highlight \`380px\` as well.`}>skip the last line</Sidenote>,
...
> I have been using
<Sidenote note={`Page is [still online](https://www.twinhelix.com/css/iepngfix/) <3`}>TwinHelix script</Sidenote>
for quite some time.
>
> Now I'm using
<Sidenote note={`Unfortunately the link is dead.`}>this one</Sidenote>
...
<Sidenote note={`Nor did CSS border radius, shadows, gradiends, transforms...`}>IE</Sidenote>
.
