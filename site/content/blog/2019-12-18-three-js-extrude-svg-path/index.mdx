...
comments = [
  "comments/three-js-extrude-svg-path/1585509760335.json",
  "comments/three-js-extrude-svg-path/1585512951827.json",
  "comments/three-js-extrude-svg-path/1599565312731.json",
  "comments/three-js-extrude-svg-path/1599566159579.json"
]
image = "./img/final.png"
...
Replaced `path.toShapes(true)` with `THREE.SVGLoader.createShapes(path)` because `toShapes` misses some of the SVG-specific logic implemented in the loader. Thanks to forresto who [pointed it out](https://github.com/Stanko/Stanko.github.io/issues/456). Also updated the demo to use a fresh version of ThreeJS.
...
Three.js provides us with [SVGLoader](https://threejs.org/docs/#examples/en/loaders/SVGLoader), but it is not the part of the main export. It is an addon, and can import it like this:
...
<svg width="202px" height="202px" viewBox="0 0 202 202" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" className="image">
...
To get to the shapes we can extrude, we need to parse the SVG. Get paths' data by calling `.paths()` method. It will return an array of [ShapePaths](https://threejs.org/docs/#api/en/extras/core/ShapePath). Finally to get an array of `Shape` objects we need to call `SVGLoader.createShapes(path)` method on which we can use `ExtrudeGeometry` on.
...
<Image
  ...
  alt="SVG path rendered in 3d space, but inverted"
/>
...
<Image
  ...
  alt="SVG path rendered in 3d space, this time in correct orientation"
/>
...
<Image
  ...
  alt="Our element rotates around it's corner and it's center"
/>
...
<Image
  src="./img/final.png"
  alt="Our element now rotates around it's center"
/>
...
<Codepen id={"gObMepb"} title={"three.js extrude SVG path"} height={420} />
