...
<Sidenote note={`If you set second parameter \`noHoles\` to \`true\` it will return all holes as separate shapes. Read more in [toShapes documention](https://threejs.org/docs/#api/en/extras/core/ShapePath.toShapes).`}>which in our case will always include only one item.</Sidenote>

Finally we got to the shapes we can use `ExtrudeGeometry` on.
