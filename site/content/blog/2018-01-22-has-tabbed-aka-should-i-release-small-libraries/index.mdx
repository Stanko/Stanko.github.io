...
<Sidenote note={`By default \`--tabbed\` is used, but it accepts custom class as parameter.`}>CSS class</Sidenote>
to `html` element.
...
<Sidenote note={`It should do it right though.`}>one small thing</Sidenote>.
