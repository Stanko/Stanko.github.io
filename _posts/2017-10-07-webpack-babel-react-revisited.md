...
<sup>1</sup> We have big plans for Marvin in the near future.
</small>
...
We need to add config we just used - only entry and output paths. <sup>2</sup>

<small class="Footnote">
<sup>2</sup>
Plus `paths` contstant. I like to keep paths in the constant object, as it makes things easier to read.
</small>
...
* Babel env preset <sup>3</sup>
* Babel React preset

<small class="Footnote">
<sup>3</sup> `babel-preset-env` is successor of `babel-preset-es2015`
...
And if we run `npm run build` you'll see `style.bundle.css`<sup>4</sup> created in the `dist` folder,
next to js and html files.

<small class="Footnote">
<sup>4</sup> I use `.bundle` postfix so you can easier differentiate webpack bundles from the source files.
</small>
...
[Commander Keen](https://raw.githubusercontent.com/Stanko/webpack-babel-react-revisited/master/src/assets/keen.png)<sup>5</sup> image in it.

<small class="Footnote">
<sup>5</sup> I still love [Commander Keen](https://en.m.wikipedia.org/wiki/Commander_Keen) games.
</small>
